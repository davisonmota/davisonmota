## I'm a software developer
Desenvolvedor back-end com interesse no universo Web3/Blockchain.

## 🧠 Sobre
* **Stack principal:** Node.js, TypeScript, Nest.js, Jest, PostgreSQL, Docker.
* **Objetivo:** Aprofundar meu conhecimento em desenvolvimento back-end e me aprofundar no universo Web3/Blockchain.

## Principais Projetos
### 1. RealWorld app: Conduit
RealWorld é um projeto da vida real, uma aplicação de blogging completa, incluindo autenticação, gerenciamento de artigos, comentários e perfis de usuários.
* **Implementação da API Rest**, seguindo a especificação do projeto.
* **Documentação com Swagger**
* **Modelagem do banco de dados relacional**
* **Testes e Deploy**
  
**Stack:** Nest.js, TypeScript, TypeOrm, PostgreSQL, Docker, Jest

___
[![My Skills](https://skillicons.dev/icons?i=nodejs,nestjs,ts,js,jest,vitest,docker,git,linux,bash,postgres,mongodb,graphql,express&theme=dark)](https://www.linkedin.com/in/davisonmota/)
